...
{
    public partial class frmExportAllocation : DevExpress.XtraEditors.XtraForm
    {
        string UserName;
        public frmExportAllocation()
        {
            InitializeComponent();
            UserName = saveConfig.UserName;
        }
        saveConfigRegedit saveConfig = new saveConfigRegedit();
        LayerLogicData.lgNote_ExportAllocation lgAllocation = new LayerLogicData.lgNote_ExportAllocation();
        LayerObjectData.objNote_ExportAllocation objAllocation = new LayerObjectData.objNote_ExportAllocation();
        ...
        {
            gridDeparment.Properties.DataSource = lgDepartment.ShowListData();
            gridDeparment.EditValue = gridDeparment.Properties.GetKeyValue(0);
        }
        ...
        public void ShowDetailNote()
        {
            ...
            {
                gridHangHoa.DataSource = lgNote_Detail.Show_ExportAllocation("-1");
                DataTable dt = (DataTable)gridHangHoa.DataSource;
                gridView1.AddNewRow();
                gridView1.FocusedColumn = gridView1.VisibleColumns[0];
            }
        ...
        public void ReadOnlyControl(bool kt)
        {
            ...
            txtDeliveryPersion.ReadOnly = kt;
            gridDeparment.ReadOnly = kt;
        ...
        public bool CheckParamemter(LayerObjectData.objNote_ExportAllocation obj)
        {
            if (obj.Code == "")
            {
        ...
        private void windowsUIButtonPanel1_ButtonClick(object sender, DevExpress.XtraBars.Docking2010.ButtonEventArgs e)
        {
            ...
            {               
                DeleteRowsEmpty();
                SetParameter_Procedure(objAllocation);
                if (CheckParamemter(objAllocation) == true)
                {
                    if (ID == null)
                    {
                        if (lgAllocation.InsertData(objAllocation) == true)
                        {
                            ID = lgAllocation.GetID(objAllocation.Code);
                            AddPIDToList(ID);
                            DataTable data = (DataTable)gridHangHoa.DataSource;
                            int dong = data.Rows.Count;
                            lgNote_UpdateData.NoteDataDetail(data, "P_Note_Export", ID);
                            ShowDetailNote();
                    ...
                    {
                        if (lgAllocation.UpdatetData(objAllocation) == true)
                        {                          
                            AddPIDToList(ID);
                            DataTable data = (DataTable)gridHangHoa.DataSource;
                            lgNote_UpdateData.NoteDataDetail(data, "P_Note_Export", ID);
        ...
        public void GetDataFind(string id, string code, string dateaction, string idsupplier, string detail, string receiver, string delivery_person)
        {
            ...
            txtDeliveryPersion.Text = delivery_person;
            gridDeparment.EditValue = idsupplier;
        ...
        private void gridSupplier_EditValueChanged(object sender, EventArgs e)
        {
            try
            {
                IDDepartment = gridDeparment.EditValue.ToString();
            }
            catch 
            {
                IDDepartment = null;
        ...
        public void SetParameter_Procedure(LayerObjectData.objNote_ExportAllocation obj)
        {
            obj.ID = ID;
            ...
            obj.Receriver = txtReceiver.Text;
            obj.Delivery_Persion = txtDeliveryPersion.Text;
        }
