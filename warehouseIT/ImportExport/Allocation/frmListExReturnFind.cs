...
{
    public partial class frmListExReturnFind : DevExpress.XtraEditors.XtraForm
    {
        public frmListExReturnFind()
        {
            InitializeComponent();
        }
        ...
        string ID, Code, DateAction, Detai,  IDDepartment;
        public delegate void SetDataFrom(string id, string code, string date,  string idsupplier, string detail);
        ...
        private void frmListReceiptFind_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                if (ID != null)
                {
                    this.setdataFrm(ID, Code, DateAction, IDDepartment, Detai);
                    this.Close();
                }
            }
        }
